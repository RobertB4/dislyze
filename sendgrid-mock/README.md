...
```
docker run -p 7000:7000 -env SENDGRID_API_KEY=YOUR_API_KEY -env SESSION_COOKIE_SECRET=YOUR_SECRET  -t yudppp/simple-sendgrid-mock-server
```

The port `7000` is for HTTP.
...
```
> curl http://localhost:7000/json?token=YOUR_API_KEY
[
  {
    "personalizations": [
      {
