<script lang="ts">
	...
	function updateURL(
		...
		goto(resolve(`/settings/users?${params.toString()}` as any), {
			replaceState: false,
			invalidate: [
				(url: URL) => {
					...
					<div class="flex-grow space-y-6">
						...
						<div class="space-y-3">
							<div
								...
								data-testid="role-selection-label"
							>
								ロール (複数選択可)
							</div>
							{#if $errors.roleIds?.[0]}
								<div class="text-sm text-red-600" data-testid="roleIds-error">
									{$errors.roleIds[0]}
								</div>
							{/if}
				...
				>
					<div class="flex-grow space-y-6">
						...
						<div class="space-y-3">
							<div
								class="block text-sm font-medium text-gray-700"
								data-testid="edit-role-selection-label"
							>
								ロール (複数選択可)
							</div>
							{#if $editErrors.roleIds?.[0]}
								<div class="text-sm text-red-600" data-testid="edit-roleIds-error">
									{$editErrors.roleIds[0]}
								</div>
							{/if}
							<div
								class="grid grid-cols-1 md:grid-cols-2 gap-3"
								data-testid="edit-role-cards-container"
							>
								{#each roles as role (role.id)}
									<RoleCard
										{role}
