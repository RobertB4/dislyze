...
```bash
npm run check     # Fast static checks: TypeScript/Svelte type checking
...
- **Build before checking frontends.** Frontends import from zoroark's `dist/` — run `npm run build` before checking frontend types.
