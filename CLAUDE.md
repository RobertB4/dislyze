...
When changing a shared resource, verify all consumers still work by running `make check` / `npm run check` in each affected consumer service.

## Definition of Done

1. **Behavior verified**: The feature works — confirmed by running integration tests/e2e tests (`make verify` or `npm run verify` in the service) AND by using `playwright-cli` to verify frontend changes in the browser. "It compiles" is not verification.
