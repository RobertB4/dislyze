...
import { resetAndSeedDatabase, closeDbPool } from "../setup/helpers";
...
test.describe("Auth - Signup Page", () => {
	...
	test("should display validation errors for all required fields if empty", async ({ page }) => {
		...
		await expect(page.locator("#user_name + p.mt-1.text-sm.text-red-600")).toContainText(
			"氏名は必須です"
		);
		await expect(page.locator("#email + p.mt-1.text-sm.text-red-600")).toContainText(
			"メールアドレスは必須です"
		);
		await expect(page.locator("#password + p.mt-1.text-sm.text-red-600")).toContainText(
			"パスワードは必須です"
		);
		await expect(page.locator("#password_confirm + p.mt-1.text-sm.text-red-600")).toContainText(
			"パスワードを確認してください"
		);
	});
