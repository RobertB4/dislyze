	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedStatus == http.StatusOK {
				...
				attemptLoginResp := attemptLogin(t, setup.TestUsersData[tt.loginUserKey].Email, tt.requestBody.NewPassword)
				defer func() {
				...
				attemptOldLoginResp := attemptLogin(t, setup.TestUsersData[tt.loginUserKey].Email, setup.TestUsersData[tt.loginUserKey].PlainTextPassword)
				defer func() {
...
func TestVerifyChangeEmail_Integration(t *testing.T) {
	...
	validUser := setup.TestUsersData["alpha_admin"]
	...
	t.Run("token reuse fails", func(t *testing.T) {
		reuseUser := setup.TestUsersData["beta_admin"]
